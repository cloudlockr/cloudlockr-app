...
describe('PostRegistrationService tests', () => {
  beforeEach(() => {
  ...
  it('registers the user with the server and stores the returned user data', async () => { 
    let execution = PostRegistrationService(email, password, password, testDispatch);
    mockAxios.mockResponse({ status: 200, data: token });
    await execution;

    // Check API request was made as intended
    expect(mockAxios.post).toHaveBeenCalledWith(`user/register`, {}, { 
        headers: {
            'email': email,
            'password': password,
            'password1': password,
